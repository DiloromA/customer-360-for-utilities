...
# MAGIC %md
# MAGIC # Score all customers for EV likelihood
# MAGIC
# MAGIC Loads `@champion` from UC, applies it to every row of
# MAGIC `ml_ev_detection_features`, and writes per-customer EV probabilities
# MAGIC to `ml_ev_detection_predictions`. Consumed by the app's customer
# MAGIC drawer to surface a "likely unregistered EV" flag when the prediction
# MAGIC disagrees with the DER record.
...
# "flag" with the actual EV prevalence (~3.2%) instead of using a hard
# 0.5 cutoff that's miscalibrated for imbalanced data.
base_rate = features_df["has_ev_label"].mean()
top_k = int(round(base_rate * len(ev_proba)))
threshold = float(pd.Series(ev_proba).nlargest(top_k).min()) if top_k > 0 else 0.5
print(f"Base rate {base_rate:.1%} → flagging top {top_k} (threshold={threshold:.3f})")

predictions = pd.DataFrame({
    "customer_id":     features_df["customer_id"].values,
    "ev_probability":  ev_proba,
    "ev_likely_flag":  (ev_proba >= threshold).astype(int),
    "has_ev_label":    features_df["has_ev_label"].values,  # for offline eval
})
...
(
    ...
    .saveAsTable(f"{catalog}.{schema}.ml_ev_detection_predictions")
)
print(f"Wrote {len(predictions)} predictions to ml_ev_detection_predictions")
print(f"Predicted EV rate: {predictions['ev_likely_flag'].mean():.1%}")
print(f"Actual EV rate (validation): {predictions['has_ev_label'].mean():.1%}")

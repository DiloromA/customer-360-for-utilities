...
-- usage_point string); service_point_number is the natural key (usage_point_id).
...
COMMENT 'Service Point dimension — CIM UsagePoint (point of delivery) plus its service location. One row per metered service point. Meter attributes are split into dim_meter (joined over time via meter_installation). service_point_id is the durable BIGINT key; service_point_number is the natural usage_point id.'
AS
SELECT
  abs(xxhash64(up.usage_point_id))        AS service_point_id,
  up.usage_point_id                  AS service_point_number,
...
FROM ${customer_master_schema}.raw_usage_point up
LEFT JOIN ${customer_master_schema}.raw_service_location sl USING (service_location_id);

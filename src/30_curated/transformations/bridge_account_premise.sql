...
-- turnover over time. One open link per premise for the current customer;
-- premises with turnover history also carry closed prior-customer links that
-- end on the successor's move-in date — a link window that overlaps the fact
-- window is what splits a premise's facts across customers.
--
...
COMMENT 'Account<->Premise effective-dated bridge. Captures which account was billing-responsible for a premise during which [link_start_date, link_end_date) window — i.e. move-in/move-out and tenant turnover. is_current marks the live link; closed links carry a prior customer''s tenancy window. account_premise_link_id is the durable BIGINT key.'
AS
SELECT

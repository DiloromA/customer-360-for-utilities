...
-- Keyed to the physical spine (premise/service point); the affected party is the
-- premise's CURRENT customer (via premise_customer_map). Tenancy is stable
-- across the fact window, so one current customer per service point — EXCEPT a
-- sub-metered commercial premise, whose 2-5
-- service points would otherwise fan this join out to duplicate rows for the

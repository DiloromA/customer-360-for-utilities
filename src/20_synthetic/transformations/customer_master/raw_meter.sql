-- Meter (raw) — CIM EndDeviceAsset, specifically the physical meter asset.
-- Realistic utility meter fleet: Itron and Landis+Gyr dominate the AMI fleet;
-- older AMR meters (Sensus, Honeywell, GE) round it out.
--
-- install_date is anchored to premise_service_attrs.in_service_date with a
-- meter-replacement jitter — the utility's AMI rollout completed around 2014, so
-- smart meters generally install in 2010-2018 even if the service itself goes
-- back further.
--
-- METER SWAPS (hierarchy nuance): most service_points have ONE meter that is
...
-- service_point. This is what makes the service_point ↔ meter relationship
-- many-to-one OVER TIME, modeled explicitly by meter_installation. The swap
-- cohort is deterministic via hash(service_point_id, 'meter_swap', seed), so the
-- two emitters (this table and meter_installation) agree without shared state.

CREATE OR REFRESH MATERIALIZED VIEW raw_meter (
...
COMMENT 'Meter — CIM EndDeviceAsset (physical meter). One or more meters per service_point: every service_point has an original meter, and a ~7% cohort of pre-2017 smart meters also has a replacement meter installed during the 2017-2018 swap window (original then has status=replaced). Realistic utility fleet manufacturer mix (Itron / Landis+Gyr / Sensus / Honeywell / GE). meter_seq orders meters at a service_point (1=original, 2=replacement); meter_installation turns these into an effective-dated bridge. PK: meter_number. FK: service_point_id -> raw_service_point. CIM: EndDeviceAsset.'
AS
...
WITH base AS (
  SELECT
    ...
    sl.in_service_date,
    abs(xxhash64(up.service_point_id, 'manufacturer', ${random_seed})) / CAST(9223372036854775807 AS DOUBLE) AS r_mfg,
    abs(xxhash64(up.service_point_id, 'install',      ${random_seed})) / CAST(9223372036854775807 AS DOUBLE) AS r_install,
    abs(xxhash64(up.service_point_id, 'serial',       ${random_seed}))                                       AS r_serial
  FROM raw_service_point up
  JOIN raw_premise_service_attrs sl ON up.service_location_id = sl.service_location_id
),

-- Original meter attributes (one per service_point), plus the swap decision.
meters AS (
  SELECT
    service_point_id,
    is_smart_meter,
    ...
    abs(xxhash64(service_point_id, 'meter_swap', ${random_seed})) % 100        AS swap_pct,
    DATE_ADD(MAKE_DATE(2017, 1, 1),
             CAST(abs(xxhash64(service_point_id, 'swap_date', ${random_seed})) % 730 AS INT)) AS swap_date
  FROM base
),
...
originals AS (
  SELECT
    -- Hash seed '_meter' is intentionally preserved — changing it re-keys the
    -- entire meter spine and every downstream surrogate that derives from it.
    md5(CONCAT(service_point_id, '_meter'))          AS meter_number,
    service_point_id,
    1                                                AS meter_seq,
    ...
    CONCAT(
      CASE
        ...
        ELSE                   '593'   -- GE
      END,
      LPAD(CAST(abs(r_serial) % 10000000 AS STRING), 7, '0')  -- r_serial from xxhash64
    )                                                AS serial_number,
...
replacements AS (
  SELECT
    -- Hash seed '_meter_2' intentionally preserved — matches meter_installation.
    md5(CONCAT(service_point_id, '_meter_2'))        AS meter_number,
    service_point_id,
    2                                                AS meter_seq,
    ...
    CONCAT(
      ...
      LPAD(CAST(abs(xxhash64(service_point_id, 'serial_2', ${random_seed})) % 10000000 AS STRING), 7, '0')
    )                                                AS serial_number,
...
SELECT meter_number, service_point_id, meter_seq, is_replacement,

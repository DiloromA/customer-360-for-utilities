-- Premise Service Attributes — premise-level CIS attributes a utility carries
-- for each service address (formatted address, service class, in-service date,
-- service status). One row per premise. The table is deliberately NOT collapsed
-- into raw_premises because service_location_id is a load-bearing surrogate
-- key that raw_service_point derives its own id from; collapsing the table
-- would re-key the service-point spine and every fact hanging off it.
--
-- This is the table that the curated layer joins to FEMA's footprint
-- geometry for map rendering — premise_id is the bridge.

CREATE OR REFRESH MATERIALIZED VIEW raw_premise_service_attrs (
...
COMMENT 'Premise Service Attributes — CIM ServiceLocation attributes for each premise. One row per premise with formatted service address, service class, in-service date, and service status. service_location_id is the load-bearing surrogate key raw_service_point derives its own id from (do not rename it). PK: service_location_id. FK: premise_id -> premises. CIM: ServiceLocation.'
AS
...
SELECT
  ...
  -- Service class — mirrors occupancy_class but is the customer-facing CIS classification.

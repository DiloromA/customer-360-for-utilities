...
// Auth strategy: prefer the app's own service principal (OAuth M2M via the
// DATABRICKS_CLIENT_ID/SECRET that Databricks Apps inject). An SP token gets the
// `all-apis` scope, which the Genie API requires — whereas the forwarded
// end-user token is downscoped to the app's declared user scopes and lacks
// `genie`. Falls back to the forwarded user token, then DATABRICKS_TOKEN (dev).
